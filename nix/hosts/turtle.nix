{ config, pkgs, ... }:
{
  ...
  networking.interfaces.ens32.ipv4.addresses = [
    { address = "192.168.1.65"; prefixLength = 24; }
  ];
  ...
  boot.zfs.devNodes = "/dev/disk/by-partlabel"; # /dev/disk/by-id is the default, but we want to make sure just in case it changes, as this is important so the pool doesn't get imported out of order
